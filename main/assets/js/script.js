(function($) {
  ...
  if ($('.shop-details .bxslider').length) {
   ...
   ========================================================================== */
	...
	// language dropdown (delegated for dynamically loaded header components)
	function mapLangLinks($dropdown){
		var path = window.location.pathname.toLowerCase();
		var file = path.split('/').pop() || 'index.html';

		var base = 'index';
		if(/^shop(?:-(en|cn))?\.html$/.test(file)){
			base = 'shop';
		}

		var map = {
			index: {vi:'index.html', en:'index-en.html', cn:'index-cn.html'},
			shop: {vi:'shop.html', en:'shop-en.html', cn:'shop-cn.html'}
		};

		if(map[base]){
			$dropdown.find('.lang-dropdown-menu a[data-lang]').each(function(){
				var lang = $(this).data('lang');
				if(map[base][lang]){
					$(this).attr('href', map[base][lang]);
				}
			});
		}
	}

	$(document).on('click', '.lang-dropdown-toggle', function(e){
		e.preventDefault();
		e.stopPropagation();
		var $dropdown = $(this).closest('.lang-dropdown');
		mapLangLinks($dropdown);
		$('.lang-dropdown').not($dropdown).removeClass('open');
		$dropdown.toggleClass('open');
	});

	$(document).on('click', '.lang-dropdown-menu', function(e){
		e.stopPropagation();
	});
